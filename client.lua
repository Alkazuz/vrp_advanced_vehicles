...
vRP = Proxy.getInterface("vRP")
vRPNserver = Tunnel.getInterface("vrp_advancedvehicles")

-----------------------------------------------------------------------------------------------------------------------------------------
-- CORE
-----------------------------------------------------------------------------------------------------------------------------------------	

RegisterNetEvent('vrp_advanced_vehicles:LixeiroCB')
AddEventHandler('vrp_advanced_vehicles:LixeiroCB', function(ret)
	if ret ~= false then
		vehicleData = ret[1]
	else
...
RegisterNetEvent('vrp_advanced_vehicles:setGlobalVehicleHandling')
AddEventHandler('vrp_advanced_vehicles:setGlobalVehicleHandling', function(ret)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if Config.allVehicles or IsThisModelACar(GetEntityModel(veh)) and veh ~= 0 then
			if seat == ped and vehicleData == nil then
				...
				TriggerServerEvent('vrp_advanced_vehicles:getVehicleData',vehicleData)
				...
				if vehicleData.loaded then
					...
					TriggerServerEvent('vrp_advanced_vehicles:setGlobalVehicleHandling',vehicleData.name, vehiclesHandlingsOriginal[vehicleData.name])
				end
			...
			if vehicleData and vehicleData.loaded then
				TriggerServerEvent('vrp_advanced_vehicles:setVehicleData',vehicleData,vehiclesHandlingsDamaged[vehicleData.name] or {})
...
RegisterNetEvent('vrp_advanced_vehicles:repairCar')
AddEventHandler('vrp_advanced_vehicles:repairCar', function(repair)
	for k,v in pairs(Config.repair[repair].repair) do
		...
		elseif v == "body" then
			...
			if vehiclesHandlingsOriginal[vehicleDataAction.name] and vehiclesHandlingsOriginal[vehicleDataAction.name][v] then
				...
				TriggerEvent('vrp_advanced_vehicles:showStatusUI')

				TriggerServerEvent('vrp_advanced_vehicles:setVehicleData',vehicleDataAction,vehiclesHandlingsDamagedAction[vehicleDataAction.name] or {})
			end
		end
	end
end)

RegisterNetEvent('vrp_advanced_vehicles:upgradeCar')
AddEventHandler('vrp_advanced_vehicles:upgradeCar', function(upgrade,idname)
	if upgrade.type == "CHandlingData" then
		...
		if vehiclesHandlingsOriginal[vehicleDataAction.name] and vehiclesHandlingsOriginal[vehicleDataAction.name][upgrade.handId] then
			...
			TriggerServerEvent('vrp_advanced_vehicles:setGlobalVehicleHandling',vehicleDataAction.name,vehiclesHandlingsOriginal[vehicleDataAction.name])
		end
...
RegisterNetEvent('vrp_advanced_vehicles:removeUpgrade')
AddEventHandler('vrp_advanced_vehicles:removeUpgrade', function(upgrade)
	if upgrade.type == "CHandlingData" then
		local handFloat = 0
		if not vehiclesHandlingsOriginal[vehicleDataAction.name] then vehiclesHandlingsOriginal[vehicleDataAction.name] = {} end
		if vehiclesHandlingsOriginal[vehicleDataAction.name] and vehiclesHandlingsOriginal[vehicleDataAction.name][upgrade.handId] then
		...
		for k,v in pairs(upgrades) do
			...
			if arr[k].improvements.powered_wheels then
				...
				if not vehiclesHandlingsOriginal[name]['powered_wheels'] then
					...
					TriggerServerEvent('vrp_advanced_vehicles:setGlobalVehicleHandling',name,vehiclesHandlingsOriginal[name])
				end
			end
			-- Set nitro
			if arr[k].improvements.type == 'nitrous' then
				if vehicleData and vehicleData.loaded == true and vehicleData.nitroAmount == 0 and vehicleData.nitroRecharges > 0 then
					TriggerEvent('vrp_advanced_vehicles:startNitroRecharge')
				end
			end
		end
	end
...
RegisterNetEvent('vrp_advanced_vehicles:showStatusUI')
AddEventHandler('vrp_advanced_vehicles:showStatusUI', function()
	if vehicleData ~= nil and vehicleData ~={} and vehicleData.loaded == true then
		...
		TriggerEvent("vrp_advanced_vehicles:Notify","negado",Lang[Config.lang]['invalid_veh'])
	end
end)
...
RegisterNUICallback('makeAction', function(data, cb)
	...
	--[[for _, jobs in ipairs(Config.Jobs) do
		local carteira,banco,job = vRPNserver.PlayerInfo()
		if (job ~= nil and job ~= 0 and job == jobs) then
			ismechanic = true
		end
	end]]
	if ismechanic or true then
	--if Config.permission == false or (ESX.PlayerData.job and ESX.PlayerData.job.name == Config.permission) then
		...
		TriggerServerEvent('vrp_advanced_vehicles:makeAction',vehicleData,data,true)
	else
		TriggerEvent("vrp_advanced_vehicles:Notify","negado",Lang[Config.lang]['no_permission'])
	end
end)
...
RegisterNUICallback('notifyOil', function(data, cb)
	TriggerEvent("vrp_advanced_vehicles:Notify","importante",Lang[Config.lang]['services_notify'])
end)
...
RegisterNetEvent('vrp_advanced_vehicles:useTheJackFunction')
	...
	if firstStep and Config.UseT1gerMechanic then
		...
		while veh == 0 and distance < 3.0 do
			...
			if IsControlJustPressed(0, 38) then
				...
				if done then
					TriggerServerEvent('vrp_advanced_vehicles:makeAction',vehicleDataAction,data,false)
...
RegisterNetEvent('vrp_advanced_vehicles:startNitroRecharge')
AddEventHandler('vrp_advanced_vehicles:startNitroRecharge', function()
	local timer = Config.NitroRechargeTime
	TriggerEvent("vrp_advanced_vehicles:Notify","importante",Lang[Config.lang]['nitro_recharging'])
	...
	if vehicleData and vehicleData.loaded and vehicleData.veh == tempveh and vehicleData.nitroAmount == 0 and vehicleData.nitroRecharges > 0 then
		...
		TriggerEvent("vrp_advanced_vehicles:Notify","importante",Lang[Config.lang]['nitro_ready'])
	end
end)
...
local function NitroLoop(lastVehicle)
  ...
  if lastVehicle ~= 0 and lastVehicle ~= vehicle then
    ...
    TriggerServerEvent('vrp_advanced_vehicles:__sync', false, false, true)
  end
  ...
  if vehicleData and vehicleData.loaded and vehicleData.nitroAmount > 0 then
		...
		if isDriving then
			...
			if vehicleData.nitroAmount == 0 then
				if vehicleData.nitroRecharges > 0 then
					...
					TriggerServerEvent('vrp_advanced_vehicles:removeNitroUpgrade',vehicleData)
					TriggerEvent("vrp_advanced_vehicles:Notify","negado",Lang[Config.lang]['nitro_empty'])
				end
			end
			if not isBoosting then
				...
				TriggerServerEvent('vrp_advanced_vehicles:__sync', true, false, false)
			end
		else
			if not isPurging then
				...
				TriggerServerEvent('vrp_advanced_vehicles:__sync', false, true, false)
			end
		end
	elseif isBoosting or isPurging then
		...
		SetVehicleLightTrailEnabled(vehicle, false)
		SetVehicleNitroPurgeEnabled(vehicle, false)
		TriggerServerEvent('vrp_advanced_vehicles:__sync', false, false, false)
	end
  elseif isBoosting or isPurging then
	SetVehicleNitroBoostEnabled(vehicle, false)
	SetVehicleLightTrailEnabled(vehicle, false)
...
RegisterNetEvent('vrp_advanced_vehicles:__update')
AddEventHandler('vrp_advanced_vehicles:__update', function (playerServerId, boostEnabled, purgeEnabled, lastVehicle)
