...
window.addEventListener('message', function (event) {
    ...
    if (item.showmenu) {
        ...
        if (item.oilHealth <= 5) {
            if (notifyOil == false) {
                notifyOil = true
                $.post("http://vrp_advanced_vehicles/notifyOil");
            }
...
function makeAction(event) {
    ...
    $.post("http://vrp_advanced_vehicles/makeAction", JSON.stringify({action:action,idname:idname,name:name,img:img}));
}

function repairCar(repair) {
    $.post("http://vrp_advanced_vehicles/repairCar", JSON.stringify({repair:repair}));
}

// FUNÇÃO PARA FECHAR A NUI 

function closeNui() {
    $.post("http://vrp_advanced_vehicles/close");
}
